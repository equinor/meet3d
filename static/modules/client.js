...
exports.appendConnectionHTMLList = appendConnectionHTMLList;
exports.addLocalTracksToConnection = addLocalTracksToConnection;
exports.addVideoStream = addVideoStream;
exports.addScreenCapture = addScreenCapture;
exports.advertiseFile = advertiseFile;
exports.dataChannelReceive = dataChannelReceive;
exports.removeVideoStream = removeVideoStream;
exports.userLeft = userLeft;
exports.updateShareScreen = updateShareScreen;
exports.openVideoPage = openVideoPage;
exports.open3D = open3D;
exports.openChat = openChat;
exports.shareCamera = shareCamera;
exports.shareScreen = shareScreen;
exports.sendChat = sendChat;
exports.shareButton = exports.sharing = void 0;

var _D = require("./3D.js");
...
var cameraButton = document.getElementById("cameraButton"); // These two variables are present in both client.js and connect.js
...
async function getLocalTrack(constraint) {
  try {
    ...
    if (e.name === "NotAllowedError") {
      alert('Unfortunately, access to your device is necessary in order to use this feature. ' + 'Permissions for this webpage can be updated in the settings for your browser, ' + 'or by refreshing the page and trying again.');
...
function stopShareCamera(button) {
  ...
  if (videoElement.children[0].children.length == 0) {
    ...
    (0, _D.resizeCanvas)(0); // Make space for the videos on the screen
  }
}
/**
...
async function shareScreen(button) {
  ...
  try {
    ...
    if (e.name === "NotAllowedError") {
      // We were not given permission to use the screen capture
      alert('Unfortunately, access to the screen is necessary in order to use the program. ' + 'Permissions for this webpage can be updated in the settings for your browser, ' + 'or by refreshing the page and trying again.');
    } else {
      console.error(e);
      alert('Unable to access local media: ' + e.name);
    }
  ...
  if (sharing.id) {
    // If someone else starts sharing whilst we select our screen, use theirs
  ...
  (0, _D.updateShareScreen3D)(screenCapture.getVideoTracks()[0], sharing); // Add the stream to the 3D environment

  if (screenCapture.getAudioTracks()[0]) (0, _D.updateShareScreen3D)(screenCapture.getAudioTracks()[0], sharing); // Add the stream to the 3D environment
...
function addScreenCapture(id) {
  if (sharing.id && sharing.id == ourID) {
    // If we are sharing
    ...
    if (id) {
      ...
      if (screenCapture.getAudioTracks()[0]) connections[id].connection.addTrack(screenCapture.getAudioTracks()[0], screenCapture); // Update our media stream with audio
    } else {
      ...
      for (let i in connections) {
        ...
        if (screenCapture.getAudioTracks()[0]) connections[i].connection.addTrack(screenCapture.getAudioTracks()[0], screenCapture); // Update our media stream with audio
      }
    }
  }
}
...
function stopShareScreen(button) {
  ...
  sharing.width = 0;
  sharing.height = 0;
  (0, _D.updateShareScreen3D)(null, sharing); // Re-add the 3D walls without the video texture
...
function updateShareScreen(videoStream) {
  if (sharing.id) {
    (0, _D.updateShareScreen3D)(videoStream, sharing);
  }
}
/**
...
function dataChannelReceive(id, data) {
  ...
  if (message.type == "pos") {
    // It is 3D positional data
    (0, _D.changeUserPosition)(id, message.x, message.y, message.z); // Change position of user
  } else if (message.type == "file") {
    // It is a list of advertised files
    ...
    if (message.sharing) {
      // If the person has started sharing
...
function addChat(name, message, whisper) {
  ...
  let second = today.getSeconds(); // Make sure that there are always two digits to each time value
  ...
  if (whisper) {
    // If the message is just to us then let the user know
...
function sendChat() {
  ...
  if (message.charAt(0) == '@') {
    // Check if someone is sending the message to someone specific
...
function advertiseFile() {
  ...
  for (let i in files) {
    ...
    if (files[i].name && files[i].size) {
      // If the file has a name and size
      fileDetailsList.push({
        // Add the file to the list of advertised files
...
function updateFileList(id, message) {
  ...
  if (!file) {
    // If the file does not already have an option
  ...
  if (!userFiles) {
    // If the user does not have a list of files already
  ...
  for (let i in files) {
    if (files[i].name == option) {
      ...
      fileReader.onload = function (e) {
        // When the files is loaded into memory
...
function addVideoStream(id, track) {
  ...
  videoDisplay();
  (0, _D.resizeCanvas)(cameraConstraints.video.width); // Make space for the videos on the screen

  (0, _D.updateVideoList)(id); // Update the list of what videos to show, in 3D.js
...
function removeVideoStream(id) {
  ...
  if (videoElement.children[0].children.length == 0) (0, _D.resizeCanvas)(0); // Make space for the videos on the screen

  (0, _D.updateVideoList)(id);
}
/**
...
function openChat() {
  document.removeEventListener("keydown", _D.onDocumentKeyDown);
  document.removeEventListener("keyup", _D.onDocumentKeyUp);
...
function open3D() {
  document.addEventListener("keydown", _D.onDocumentKeyDown, false);
  document.addEventListener("keyup", _D.onDocumentKeyUp, false);
...
function initSwapView() {
  ...
  chatSend.onfocus = function () {
    document.removeEventListener("keyup", swapViewOnC);
  };

  chatSend.onblur = function () {
    document.addEventListener("keyup", swapViewOnC);
  };
}
/**
...
function swapViewOnC(event) {
  if (event.key == 'c') {
    if (_D.controls.isLocked === true) _D.controls.unlock(); // Unlocks the mouse if you swap view while moving in the 3D-space

    if (videoButton.hidden == true) return;
    if (roomButton.hidden == false) open3D();else openChat();
  }
}
/**
...
function userLeft(id) {
  ...
  if (id == sharing.id) {
    // If they were sharing their screen then remove it
    sharing.id = null;
    sharing.width = 0;
    sharing.height = 0;
    screenShare.srcObject = null;
    shareButton.hidden = false;
    (0, _D.updateShareScreen3D)(null, sharing);
  }
...
function clearHTML() {
  ...
  for (let id in connections) {
    if (connections[id].stream) connections[id].stream.getTracks().forEach(track => track.stop()); // Stop all remote media tracks, if there are any
