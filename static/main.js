...
var _D = require("./modules/3D.js");

var _client = require("./modules/client.js");
...
videoButton.onclick = function () {
  (0, _client.openVideoPage)();
};

shareButton.onclick = function () {
  (0, _client.shareScreen)(shareButton);
};

cameraButton.onclick = function () {
  (0, _client.shareCamera)(cameraButton);
};
...
chatSend.addEventListener("keyup", function (event) {
  if (event.keyCode === 13) {
    ...
    (0, _client.sendChat)(); // Send chat message by pressing enter in the chat
  }
});
...
async function init(button) {
  ...
  let startInfo = {
    room: roomName.value,
    // The room we want to join
    name: username.value // Our username

  };
  socket.emit('join', startInfo);
  console.log('Attempting to join ' + roomName.value); // The room we tried to join is full
  ...
  socket.on('join', function (message) {
    ...
    (0, _D.newUserJoined3D)(message.id, message.name); // Add the new user to the 3D environment

    (0, _client.appendConnectionHTMLList)(message.id);
  }); // We joined a conference

  socket.on('joined', async function (connectionInfo) {
    console.log('We joined: ' + connectionInfo.room);
    ourID = connectionInfo.id;
    await (0, _client.initChat)(ourID, connections);
    await (0, _D.init3D)(ourID, connections, document.getElementById("3D")); // Renders the 3D environment
  ...
  socket.on('offer', function (message) {
    ...
    if (!connections[id]) {
      ...
      (0, _client.appendConnectionHTMLList)(id); // Add their username to the list of connections on the webpage

      (0, _D.newUserJoined3D)(id, name); // Add new user to 3D environment
    }
...
async function sendOffer(id) {
  ...
  if (!connections[id].connection) {
    ...
    (0, _client.addLocalTracksToConnection)(id); // This triggers 'renegotiations'
  }
}
/**
...
async function sendAnswer(id, offerDescription) {
  if (!connections[id].connection) {
    ...
    connections[id].connection = await createPeerConnection(id);
    (0, _client.addLocalTracksToConnection)(id);
  }
  ...
  try {
    ...
    pc.ontrack = function (event) {
      ...
      if (event.track.kind == "audio") {
        ...
        (0, _D.userGotMedia)(id, newStream); // Adds audio track to 3D environment
      }

      if (event.track.kind == "video") {
        if (event.streams[0].id !== connections[id].audiostream.id) {
          // Screen capture video
          (0, _client.updateShareScreen)(event.track); // Add the video track to the 3D environment
        } else {
          // Web camera video
          // Web camera videos should always be in a stream
          (0, _client.addVideoStream)(id, event.track);
          ...
          event.streams[0].onremovetrack = function (event) {
            ...
            if (event.track.kind == "video") {
              (0, _client.removeVideoStream)(id);
            }
          };
        }
      }
    ...
    pc.ondatachannel = function (event) {
      ...
      event.channel.addEventListener("close", () => {
        ...
        (0, _D.userLeft3D)(id); // Removes the user from the 3D environment

        (0, _client.userLeft)(id);
      });
      event.channel.addEventListener("message", message => {
        (0, _client.dataChannelReceive)(id, message.data); // Called when we receive a DataChannel message
      });
    };
...
function createDataChannel(id) {
  ...
  tempConnection.onopen = function () {
    ...
    console.log("Datachannel established to " + connections[id].name);
    (0, _client.advertiseFile)();
    (0, _client.addScreenCapture)(id);
    (0, _D.updatePos)();
  };
  ...
  tempConnection.onmessage = function (event) {
    (0, _client.dataChannelReceive)(id, event.data); // Called when we receive a DataChannel message
  };
}
/**
...
function leave(button) {
  ...
  (0, _D.leave3D)(); // Closes the 3D environment
